...
module.exports = [
    ...
    new Level(
        ...
        <p>Вторая: если повторяется одна команда, то инструкцию повтора <span class="inline-code">loop</span>\
        писать совсем не обязательно, ведь можно просто написать число повторов у команды между скобками.\
        Посмотри: если написать\
             <pre><code class="js">' +
                'forward(5);\n'+
            '</code></pre>\
        и\
              <pre><code class="js">' +
                ...
                '}\n'+
            '</code></pre>\
        <p>то танк проедет 5 клеток и в первом примере, и во втором.\
        Инструкция <span class="inline-code">loop</span> нужно только для повторяющихся команд.\
        <p>Ну а сейчас выполни задание с танком используя два повтора,\
        чтобы программа получилась не очень длинной. Она всё равно длиннее чем ты писал до сих пор,\
        но только представь себе какой длинны получилась бы программа без повторов!',
